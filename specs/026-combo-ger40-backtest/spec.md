...
- Q: What session are the candles built over? → A: **02:00–22:00 Paris local**, the full GER40 CFD day — twenty hours, not the 09:00–22:00 window the impulsive GER40 variants use. It gets its own market definition rather than widening theirs, because a "bougie de 9h" strategy derives its reference candle from the session open.
...
- **Candles are built over the full GER40 CFD session — 02:00–22:00 Paris, DST-aware** — so the pre-cash-open hours, the afternoon and the US-driven evening are all visible on all three timeframes. This is a **twenty-hour** day, not the thirteen-hour 09:00–22:00 window the impulsive GER40 variants trade: those are "bougie de 9h" strategies that derive their 09:00–10:00 reference candle from the session open, so they keep their own session and are untouched by this. The 15-minute timeframe is served natively by the historical-candle source and needs no reconstruction.
