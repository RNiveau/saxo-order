...
- **FR-015**: A combo that met none of its scoring criteria MUST NOT be emitted, on either timeframe. It cleared the structural gates and nothing more, and every alert emitted costs an entry in the reasoning payload — one which the pattern ranking would present as among the strongest signals available. The asset is still scanned and its other detectors still record what they find. This changes the daily combo's existing behaviour deliberately: one rule, both timeframes.
...
- **SC-009**: No combo alert, on either timeframe, is recorded for an asset whose signal met none of its criteria, verified over a full scan — the reasoning payload carries no entry that says nothing.
