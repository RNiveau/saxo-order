...
- Q: Where does the historical data for threshold calibration and the labelled validation sample come from? → A: Calibration reads the existing backtest raw-candle store; the labelled sample of historical setups is produced by the trader before release. Both are release prerequisites, recorded under Dependencies.
...
- **FR-009**: When daily and weekly combos for one asset disagree in direction, the digest MUST report the disagreement explicitly rather than emitting a single unqualified direction, and MUST resolve it under the long-only mandate: the bearish leg counts against the long thesis and caps the asset's conviction, whichever timeframe it sits on.
...
- **FR-012**: Weekly detection MUST be able to be switched off without affecting any existing daily detection, so the scan can be reverted to its current behaviour if the new signal proves too noisy.
- **FR-013**: Existing alerts stored before this feature MUST remain readable and displayable unchanged.
- **FR-014**: Repeat suppression MUST apply to what is recorded, never to what is detected. Every scan MUST evaluate the weekly combo afresh, so the digest always reflects the state of the forming bar on the day it runs — including a direction that has changed since the alert was first recorded.
...
- **Historical weekly data for calibration**: the threshold calibration required by FR-011 reads the existing backtest raw-candle store rather than re-fetching from the market data provider. Availability of a sufficient weekly span for a representative set of assets is a release prerequisite.
...
- The digest continues to be synthesised from what each scan detects rather than from stored history, so a persisting weekly setup remains visible in the digest for as long as it holds even though it is recorded only once (FR-014).
