...
- **FR-015**: A combo that meets none of its scoring criteria MUST be reported as no combo at all, on either timeframe, rather than as a weak one. Clearing the structural gates and scoring nothing means the setup is absent, not faint. A combo that meets at least one criterion MUST still be reported, labelled weak — a weak signal is a real observation and the trader is told about it. This changes the daily detector's existing behaviour deliberately: one rule, both timeframes.
...
- **SC-009**: Every combo alert recorded, on either timeframe, met at least one scoring criterion — verified over a full scan, so no entry in the reasoning payload says nothing. A weak combo that met one criterion is still recorded.
...
- Redesigning the combo criteria themselves, or their scoring. Suppressing an unscored signal (FR-015) changes what is *emitted*, not how anything is measured. The weekly variant reuses them minus one, with re-stated strength bands (FR-004); the daily combo's criteria, scoring and bands are untouched.
