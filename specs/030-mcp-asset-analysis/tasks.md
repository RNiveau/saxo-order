...
- [ ] T009 Extract `_run_double_top` (`alerting.py:675`) and `_run_double_bottom` (`:698`) into `services/detection_service.py`, parameterised by `asset_type` instead of the hardcoded `AssetType.STOCK` at `:680`/`:703`. **Preserve both behaviours that live only in these wrappers**: the tick lookup (`get_asset_detail` + `client_helper.get_tick_size`, defaulting to `0.0` when the asset has no `TickSizeScheme`) and the **2-day recency filter** — without it a double top from three weeks ago reports as a current hit that the scan would never have alerted on
- [ ] T010 Rewire `saxo_order/commands/alerting.py` to call `services/candle_source.py` and `services/detection_service.py`, deleting the five local helpers. Scan behaviour MUST be unchanged
- [ ] T011 Prove T006–T010 changed nothing: `poetry run pytest tests/ -k "alerting or congestion or double" -v`
