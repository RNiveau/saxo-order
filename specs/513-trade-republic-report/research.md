...
| `ISIN` | B`r` | `symbol` |
| `Date` | C`r` | `date` |
| `Sens` | D`r` | `"Achat"` if `type == "BUY"`, `"Vente"` if `type == "SELL"`, blank otherwise |
| `Prix` | E`r` | `price` |
| `Quantité` | F`r` | `shares` |
| `Frais` | G`r` | `fee` |
...
**Rationale**: The trader confirmed the sheet's real name and column layout, so no placeholder is needed. `Sens` is derived directly from the CSV `type` column: the trader confirmed the full set of values Trade Republic uses (`BUY`, `SELL`, `TRANSFER_INBOUND`, `DIVIDEND`, `INTEREST_PAYMENT`, `IPO_SUBSCRIPTION`), so mapping `BUY`→`"Achat"` and `SELL`→`"Vente"` (blank for the other four, which aren't buy/sell trades) is no longer a guess — an earlier version of this decision derived `Sens` from the sign of `amount` specifically because the `type` vocabulary wasn't confirmed yet; that workaround is no longer needed now that it is (see §4 for why `type` itself still isn't turned into a full enum — only these two values feed `Sens`). `Total` (gross, `price * shares`) is distinguished from `Total TTC` (gross plus fees, matching the French accounting convention "TTC" = toutes taxes comprises), per the trader's explicit formula. Consistent with the existing `GSheetClient` pattern (e.g. `_generate_r_v_block`'s `=F{row}+R{row}+S{row}` cell formula), `Total` and `Total TTC` are written as spreadsheet formulas referencing the row's own `Prix`/`Quantité`/`Frais` cells, not as pre-computed values — so the sheet stays self-consistent if a cell is later edited by hand. This method is separate from `_generate_row` (used by the existing Saxo/Binance "Liste d'ordre" journal) since that layout encodes unrelated order semantics (stop/objective/strategy) that don't apply here — reusing it would require awkward workarounds rather than the small, purpose-built append this format actually needs.
...
- Derive `Sens` from the sign of `amount` instead of `type`: rejected (superseded) — this was the original approach while the `type` vocabulary was unconfirmed; it produced wrong results in practice (e.g. a `SELL` producing positive `amount` was fine, but non-trade cash movements have a sign too and would have been mis-labeled Achat/Vente despite not being trades at all). Now that the six `type` values are confirmed, deriving `Sens` directly from `type` is both more accurate and lets non-trade types correctly get a blank `Sens`.
