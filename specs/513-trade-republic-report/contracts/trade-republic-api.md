...
- `500` — Google Sheets write failure (connectivity/permission); response body carries a clear `detail` message. All selected transactions are written in a **single** Sheets `values.append` API call (all rows in one request body), so a failure means **none** of the selected rows were written — satisfying spec FR-005 / US2 Acceptance Scenario 2 (no partial/corrupted state). On success, `exported_count` always equals the number of transactions in the request.

**Note**: All selected transactions are appended together, one row each, to the existing "ETF / DCA" Google Sheet in a single batched request, with columns `ETF, ISIN, Date, Sens, Prix, Quantité, Frais, Total, Total TTC` mapped per spec FR-013 / research.md §6. This request/response shape carries the full `TradeRepublicTransaction`; the sheet-column mapping is applied server-side and does not change this contract if the mapping is later adjusted.
