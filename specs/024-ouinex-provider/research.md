...
- ✅ **Auth (Decision 3) — RESOLVED (2026-09-12, from https://api.ouinex.com/).** The mutation is `service_signin(service_api_key, service_api_secret) { jwt expires_at }`. `expires_at` is an epoch timestamp in **milliseconds** (values below 1e12 are seconds); the JWT is long-lived (~2 years). Verified live with the configured key.
- ✅ **Closed orders — RESOLVED (2026-09-12, key re-issued with order permissions).** `closed_orders(wallet_id: "", dateRange: {time_from, time_to}, pager: {limit, offset}, status, side)` — both `time_from` and `time_to` must be ISO 8601 (`YYYY-MM-DDTHH:MM:SSZ`; a date-only `time_from` alone returns nothing, epoch values raise `UNEXPECTED_DB_ERROR`); enums are lowercase (`buy`/`sell`, status `completed`/`cancelled`/`rejected`/`expired`). Semantics verified against the account ledger: `total` is the **net amount received** (base on a buy, quote on a sell) with fees deducted; on a buy `quantity` is the base quantity **or**, for amount-based market orders, the quote amount spent; `created_at` is the placement time (epoch ms string) and the fill time is `trades[].created_at_iso`; `dateRange` filters on placement time. `fees[] { currency_id amount }` carries the commission (base asset on buys, quote on sells). Recurring / instant buys go through `conversions(wallet_id: "", dateRange, pager) { source_currency_id source_currency_amount target_currency_id target_currency_amount price status created_at_iso }` (separate API-key permission). `price` is the quoted market rate and the amount received is net of the spread (`fee_currency_amount` is always 0), so the spread is reported as the commission. Only cash↔crypto conversions are reported as buys/sells.
