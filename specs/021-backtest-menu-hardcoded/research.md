...
## 8. Raw-candle caching, scoped to the backtest feature (added 2026-07-24; re-keyed on (instrument, session window, trading date) 2026-07-28)

**Decision**: Add a new DynamoDB table (e.g. `backtest_candle_cache`, hash key `definition_code` String, range key `trading_date` String) and a small set of methods on `client/aws_client.py::DynamoDBClient` (`get_cached_backtest_candles(definition_code, trading_date)` / `store_backtest_candles(definition_code, trading_date, h1_candle, m5_candles)`). `api/services/backtest_service.py` checks this cache before calling `CandlesService.get_candles_in_window` for a given day, and stores the result (or an explicit "no data" marker) after a cache miss. The strategy evaluation (`evaluate_day`) is unchanged — it always runs against whatever candles it is handed, cached or fresh. No TTL is set on the table.

**Rationale**: Repeated backtest runs (the trader's stated use case — "we need to run it a lot of time") re-fetch the same FRA40.I H1 and 5-minute candles for the same historical, already-closed trading days every time, which is the costly part (Saxo API calls), not the strategy evaluation itself (cheap, in-process). Caching only the raw candle data — never the computed `Trade`/`DayResult`/summary — means a later change to the tunable thresholds (FR-025) or to the strategy logic itself is reflected immediately on the next run, with no cache-invalidation scheme needed (Clarifications, Session 2026-07-24). Scoping the cache to the Backtest feature (rather than adding it to `CandlesService.get_candles_in_window` itself, which would make it available to any future caller) was an explicit choice to keep this change contained to spec 021; the accepted cost is that the two hardcoded backtests ("CAC40 Bougie de 9h" and its time-cut variant) each cache the same day's candles separately under their own `definition_code`, rather than sharing one cached copy.

**Superseded 2026-07-28 (key only)**: that accepted cost was reversed. The cache key is now `{instrument}:{session window}:v{schema version}` — exactly what the Saxo fetch depends on, with the session window carrying the exchange-local hours *and* their timezone, since that is what `market_in_utc` resolves them against — so every definition on the same instrument and session shares one entry; only the CFD-session variant (spec 025, FR-G12) keeps a separate namespace, because it fetches 5-minute candles to 22:00 rather than 17:30. The rest of this decision is unchanged: same table, same scoping to the backtest feature, still raw candles only, still no TTL. The hash key attribute keeps the name `definition_code` (renaming a DynamoDB key attribute means replacing the table); `get_cached_backtest_candles` / `store_backtest_candles` take a `cache_key` instead. A day skipped by a minimum-H1-range definition (FR-033) is stored with `m5_fetched=False` so the shared entry is completed rather than read as "no 5-minute candles"; that partial write is conditional on the key being absent, so two runs racing on the same day cannot leave the cache worse than either would alone. Existing entries are moved by `k-order internal migrate-backtest-cache`.

**Alternatives considered**:
- Cache the computed `BacktestRunResult`/`DayResult` (the final strategy output) instead of raw candles — rejected per the user's explicit clarification: this would require a cache-invalidation/versioning scheme for every future change to the strategy logic or thresholds, which raw-candle caching avoids entirely.
- Key the cache by (instrument, horizon, trading date) inside `CandlesService`, generic to any caller — considered and preferred by the assistant, but rejected by the user in favor of keeping the cache a capability of the Backtest feature specifically, keyed by (backtest definition code, trading date), accepting the resulting duplication between the two backtests.
- TTL-based expiry (e.g. 30/90 days) — rejected: the cached days are always fully closed historical trading days whose candle data does not change once cached, so there is no freshness concern a TTL would address; a manual data-maintenance action is the accepted path if a cached day is ever found wrong.
