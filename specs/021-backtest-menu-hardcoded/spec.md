...
- **Close-back too far from the H1 low, or already at/past take-profit**: A 5-minute candle can satisfy FR-006's "closes at or above the H1 low" test yet close more than 20 points above it, or at/above the take-profit level itself (FR-006a) — for example a sharp reversal candle. No trade is opened in this case; the candle does not count as a breach either, so the strategy immediately resumes looking for a fresh breakdown below the H1 low, the same as if no signal had occurred yet.
...
- **FR-002**: System MUST provide one hardcoded backtest, "CAC40 Bougie de 9h," selectable from the Backtest menu, implementing the rules in FR-003 through FR-011 (including FR-006a and FR-008a). This backtest's rules are fixed in code; the menu does not offer a generic engine for defining new strategies.
...
- **FR-006a**: A recognized signal (FR-006) only produces a trade if the closing price is both (a) no more than 20 points above the H1 low, and (b) strictly below the take-profit level (H1 high minus 10 points, FR-008). A close-back that satisfies FR-006 but fails either bound is not a valid entry: no trade is opened, and the strategy immediately resumes searching for a fresh signal (a new breakdown below the H1 low) per FR-006/FR-011, rather than waiting out the rest of the day. This prevents entries that would leave little or no room to reach take-profit, or that would already be at or past it (added after PR review).
- **FR-007**: When a valid trade signal is recognized (FR-006, FR-006a), system MUST record a long (buy) entry at the closing price of the 5-minute candle that closed at or above the H1 low.
