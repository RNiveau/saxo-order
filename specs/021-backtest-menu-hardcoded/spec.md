...
- **Candidate discarded by a fresh close below the H1 low**: While waiting for breakout confirmation, a candle that closes back below the H1 low discards the pending candidate outright (FR-006b) — because its low is then also below the H1 low, this same candle immediately counts as a new breakdown (FR-006), rather than requiring a separate candle to re-trigger one.
...
- **FR-006**: System MUST recognize a breakdown-and-reversal candidate each time, after 10:00, price trades below the H1 low and a later 5-minute candle closes at or above the H1 low, provided no trade is currently open and no candidate is already pending. That reversal candle becomes the pending candidate; closing back above the H1 low does not by itself produce a trade (see FR-006b for the breakout confirmation that does). This allows multiple candidates — and multiple trades — on the same day, as long as they occur sequentially rather than concurrently.
- **FR-006b**: A pending candidate (FR-006) only produces a trade once a later 5-minute candle's high trades above the candidate's high ("breakout confirmation"). Until that happens, each subsequent 5-minute candle is resolved as follows, in order:
  ...
  - Otherwise, if it closes back below the H1 low, the candidate is discarded and the search returns to looking for a fresh breakdown-and-reversal pair (FR-006) — this candle's close below the H1 low also means its low is below the H1 low, so it doubles as that fresh breakdown.
