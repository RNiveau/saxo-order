...
def calculate_line(line_type: LineType, candles: List[Candle], max_len: int, which_second_point: int) -> tuple[LineFormula, Candle]:
    """
    Calculate the line of the congestion indicator
    Based on the formula https://www.alloprof.qc.ca/fr/eleves/bv/mathematiques/l-equation-d-une-droite-a-partir-de-coordonnees-m1319
    Y = mX + b
    """
    tab_high = [0, 0, 0]
    tab_index = [0, 0, 0]
    ...
    for i in range(1, min(max_len, len(candles))):
        if (line_type == LineType.HIGH and candles[i].higher > first_high) or (line_type == LineType.LOW and candles[i].lower < first_high):
            ...
            first_high = candles[i].higher if line_type == LineType.HIGH else candles[i].lower
            first_index = i
    
    if tab_high[which_second_point] > 0:
        m = (tab_high[which_second_point] - first_high) / (tab_index[which_second_point] - first_index)
...
def calculate_congestion_indicator(candles: List[Candle]) -> List[Candle]:
    ...
    for back_test in range(min(50, len(candles)),  3, -1):
        ...
        line_formula, candle = calculate_line(LineType.HIGH, candles, back_test, 0)
        # drop too small line
